	...
	allTickers[allTickers.length - 1].forEach(async ({ symbol, lastPrice, price24hPcnt }) => {
		...
		if (prevTicker) {
			...
			if ((shortPerodPercent >= 5 && price24hPercent >= 5) || (shortPerodPercent <= -5 && price24hPercent <= -5)) {
				...
				await sendSignal(text, symbol, lastPrice);
			}
		}
	});
};

const sendSignal = async (text, symbol, lastPrice) => {
	try {
		const candles = await axios.post(`${SECOND_SERVER}/candles`, { symbol, interval: '1', limit: 20 });

		const buffer = await getPlotImage(candles.data, symbol, lastPrice);
		...
		console.log(error);
		sendSignal(text, symbol, lastPrice);
	}
};
